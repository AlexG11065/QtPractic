...
class Window(QtWidgets.QWidget):
    ...
    def initSignals(self) -> None:
        ...
        self.pushButtonComboBox.clicked.connect(self.onPushButtonComboBoxClicked)  # подключить слот для вывода
        # текста из comboBox в plainTextEditLog при нажатии на кнопку
        self.pushButtonLineEdit.clicked.connect(self.onPushButtonLineEditClicked)
        self.pushButtonTextEdit.clicked.connect(self.onPushButtonTextEditClicked)  # подключить слот для вывода
        # текста из textEdit в plainTextEditLog при нажатии на кнопку
        self.pushButtonPlainTextEdit.clicked.connect(self.onPushButtonPlainTextEditClicked)  # подключить слот для
        # вывода текста из plainTextEdit в plainTextEditLog при нажатии на кнопку
        self.pushButtonSpinBox.clicked.connect(self.onPushButtonSpinBoxClicked)  # подключить слот для вывода
        # значения из spinBox в plainTextEditLog при нажатии на кнопку
        self.pushButtonDoubleSpinBox.clicked.connect(self.onPushButtonDoubleSpinBoxClicked)  # подключить слот для
        # вывода значения из doubleSpinBox в plainTextEditLog при нажатии на кнопку
        self.pushButtonTimeEdit.clicked.connect(self.onPushButtonTimeEditClicked)  # подключить слот для вывода
        # времени из timeEdit в plainTextEditLog при нажатии на кнопку
        self.pushButtonDateTimeEdit.clicked.connect(self.onPushButtonDateTimeEditClicked)  # подключить слот для
        # вывода времени из dateTimeEdit в plainTextEditLog при нажатии на кнопку
        self.pushButtonClearLog.clicked.connect(self.onPushButtonClearLog)  # подключить слот для очистки
        # plainTextEditLog при нажатии на кнопку

        self.comboBox.currentTextChanged.connect(self.onComboBox)  #  подключить слот для вывода текста в plainTextEditLog при изменении выбранного элемента в comboBox
        self.spinBox.valueChanged.connect(self.onSpinBox)  #  подключить слот для вывода значения в plainTextEditLog при изменении значения в spinBox
        self.dateTimeEdit.dateTimeChanged.connect(self.onDateTimeEdit)  #  подключить слот для вывода датывремени в plainTextEditLog при изменении датывремени в dateTimeEdit
    ...
    def onComboBox(self, text: str):
        """
        Обработка сигнала для ComboBox
        :param text:
        :return:
        """
        self.plainTextEditLog.appendPlainText(text)

    def onSpinBox(self, value: int):
        """
        Обработка сигнала для SpinBox
        :param value:
        :return: None
        """
        self.plainTextEditLog.appendPlainText(str(value))

    def onDateTimeEdit(self, datetime: QtCore.QDateTime):
        """
        Обработка сигнала для DateTimeEdit
        :param datetime:
        :return: None
        """
